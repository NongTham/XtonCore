...
if (!fs2.existsSync(logDir)) {
  fs2.mkdirSync(logDir);
}
var filename = path2.join(logDir, `Client.log`);
var Clientlogger = (0, import_winston.createLogger)({
  // change level if in dev environment versus production
  level: "production" ? "info" : "debug",
  format: import_winston.format.combine(
    //@ts-igonre
    import_winston.format.label({ label: path2.basename(process.mainModule.filename) }),
    ...
    new import_winston.transports.Console({
      format: import_winston.format.combine(
        ...
        import_winston.format.printf(
          (info) => `[${gradient.rainbow(`${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)} MB`)}][${gradient.rainbow(`${info.timestamp}`)}] ${info.level} [${gradient.pastel.multiline("XtonCore")}] [${info.label}] [Client] : ${info.message}`
        )
      )
    }),
...
var ClientHandler = class {
  constructor({
    ...
    figlet("XtonCore", function(err, data) {
      if (err) {
        Clientlogger.error("Something went wrong...");
        console.dir(err);
        return;
      }
      console.log(gradient2.rainbow(data));
