...
async function buildCommandTree(commandsDir) {
  ...
  for (const commandFilePath of commandFilePaths) {
    try {
      ...
      const commandModule = __require(absolutePath);
...
async function buildCommandTreeLazy(commandsDir) {
  ...
  for (const commandFilePath of commandFilePaths) {
    try {
      const absolutePath = path3.resolve(commandFilePath);
...
async function loadCommandMetadata(filePath) {
  try {
    const commandModule = __require(filePath);
...
async function loadCommandFunction(command) {
  ...
  try {
    const startTime = Date.now();
    const commandModule = __require(command._filePath);
...
// src/logger.ts
var logDir = "Logs";
var filename = path4.join(logDir, `Client.log`);
var errorFilename = path4.join(logDir, `Error.log`);
var debugFilename = path4.join(logDir, `Debug.log`);
var Clientlogger = createLogger({
  level: process.env.NODE_ENV === "production" ? "info" : "debug",
  format: format.combine(
    format.errors({ stack: true }),
    format.label({ label: path4.basename(process.mainModule?.filename ?? "unknown") }),
...
${info.stack}`;
  ...
  exceptionHandlers: [
    new transports.File({ filename: path4.join(logDir, "exceptions.log") })
  ],
  rejectionHandlers: [
    new transports.File({ filename: path4.join(logDir, "rejections.log") })
  ]
});
...
var PerformanceManager = class {
  ...
  constructor() {
    ...
    this.statsFile = path5.join(process.cwd(), "stats", "command-stats.json");
    this.metrics = {
  ...
  async initializeStatsDirectory() {
    try {
      await fs2.mkdir(path5.dirname(this.statsFile), { recursive: true });
    } catch (error) {
  ...
  async loadStats() {
    try {
      const data = await fs2.readFile(this.statsFile, "utf-8");
  ...
  async saveStats() {
    try {
      ...
      await fs2.writeFile(this.statsFile, JSON.stringify(statsObject, null, 2));
    } catch (error) {
    ...
    try {
      ...
      for (const filePath of componentFiles) {
        try {
          ...
          const componentModule = __require(absolutePath);
...
var HotReloadManager = class {
  ...
  watchDirectory(directory, callback, options = {}) {
    ...
    try {
      ...
      const watcher = fs3.watch(directory, { recursive }, async (eventType, filename2) => {
        if (!filename2) return;
        const filePath = path7.join(directory, filename2);
        const ext = path7.extname(filename2);
...
var JobManager = class {
   ...
   */
  async loadJobs() {
    if (!fs4.existsSync(this.jobsPath)) {
      Clientlogger.warn(`Jobs directory not found at ${this.jobsPath}`);
      return 0;
    }
    this.stopAll();
    this.jobs.clear();
    const jobFiles = fs4.readdirSync(this.jobsPath).filter((file) => file.endsWith(".ts") || file.endsWith(".js"));
    let loadedCount = 0;
    for (const file of jobFiles) {
      const filePath = path8.join(this.jobsPath, file);
      try {
        delete __require.cache[__require.resolve(filePath)];
        const jobModule = __require(filePath);
...
var EnhancedEmbedBuilder = class {
  ...
  static createUserProfile(user) {
    const embed = new EmbedBuilder().setTitle(`${user.username}'s Profile`).setThumbnail(user.displayAvatarURL({ size: 256 })).setColor(39423).addFields(
  ...
  static createGuildInfo(guild) {
    const embed = new EmbedBuilder().setTitle(`${guild.name} Server Information`).setThumbnail(guild.iconURL({ size: 256 })).setColor(39423).addFields(
  ...
  static createCommandHelp(commandName, description, usage, examples = [], aliases2 = []) {
    const embed = new EmbedBuilder().setTitle(`\u{1F4D6} Command: ${commandName}`).setDescription(description).setColor(39423).addFields(
  ...
  static createPagination(items, page, itemsPerPage = 10, title = "Results") {
    ...
    const embed = new EmbedBuilder().setTitle(title).setDescription(pageItems.join("\n") || "No items found").setColor(39423).setFooter({ text: `Page ${page} of ${totalPages} \u2022 ${items.length} total items` }).setTimestamp();
    return embed;
  }
  static createStats(stats) {
    const embed = new EmbedBuilder().setTitle("\u{1F4CA} Statistics").setColor(39423).setTimestamp();
  ...
  static createProgressBar(current, max, length = 20, title) {
    ...
    const embed = new EmbedBuilder().setColor(percentage === 100 ? 65280 : 39423).addFields({
      name: title || "Progress",
...
var ComponentHelpers = class {
  ...
  static createSelectMenu(customId, placeholder, options, settings = {}) {
    const selectMenu = new StringSelectMenuBuilder().setCustomId(customId).setPlaceholder(placeholder);
  ...
  static createModal(customId, title, inputs) {
    ...
    for (const input of inputs) {
      const textInput = new TextInputBuilder().setCustomId(input.customId).setLabel(input.label).setStyle(input.style || TextInputStyle.Short);
...
var PaginationBuilder = class {
  ...
  createButtons(disabled = false) {
    const row = new ActionRowBuilder2().addComponents(
      new ButtonBuilder2().setCustomId("prev_page").setLabel("Previous").setStyle(ButtonStyle2.Secondary).setDisabled(disabled),
      new ButtonBuilder2().setCustomId("next_page").setLabel("Next").setStyle(ButtonStyle2.Primary).setDisabled(disabled)
    );
    return row;
  }
};
...
var ClientHandler = class _ClientHandler {
  ...
  async _eventsInit() {
    ...
    for (const eventPath of eventPaths) {
      const eventName = path10.basename(eventPath);
      ...
      this._client.on(eventName, async (...args) => {
        for (const eventFuncPath of eventFuncPaths) {
          try {
            const absolutePath = path10.resolve(eventFuncPath);
            const eventModule = __require(absolutePath);
  ...
  async _validationsInit() {
    ...
    for (const validationFilePath of validationFilePaths) {
      try {
        const absolutePath = path10.resolve(validationFilePath);
