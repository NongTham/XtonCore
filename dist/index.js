...
var require_errors = __commonJS({
  "node_modules/logform/errors.js"(exports, module2) {
    ...
    var format3 = require_format();
    var { LEVEL, MESSAGE } = require_triple_beam();
    module2.exports = format3((einfo, { stack, cause }) => {
    ...
    exports.warn = {
      ...
      moved(obj, movedTo, prop) {
        function movedNotice() {
          return () => {
            throw new Error([
              format3("winston.%s was moved in winston@3.0.0.", prop),
              format3("Use a winston.%s instead.", movedTo)
            ].join("\n"));
          };
        }
  ...
  "node_modules/winston/lib/winston/transports/file.js"(exports, module2) {
    ...
    module2.exports = class File extends TransportStream {
      ...
      _checkMaxFilesTailable(ext, basename, callback) {
        ...
        asyncSeries(tasks, () => {
          fs3.rename(
            path3.join(this.dirname, `${basename}${ext}`),
  ...
  "node_modules/winston/lib/winston/transports/http.js"(exports, module2) {
    ...
    module2.exports = class Http extends TransportStream {
      ...
      _request(options, callback) {
        ...
        const path3 = options.path || this.path || "";
        delete options.auth;
        delete options.path;
        if (this.batch) {
...
var filename = import_path.default.join(logDir, `Client.log`);
var Clientlogger = (0, import_winston.createLogger)({
  // change level if in dev environment versus production
  level: "production" ? "info" : "debug",
  format: import_winston.format.combine(
    //@ts-igonre
    import_winston.format.label({ label: import_path.default.basename(process.mainModule.filename) }),
  ...
  transports: [
    new import_winston.transports.Console({
      format: import_winston.format.combine(
        ...
        import_winston.format.printf(
          (info) => `[${import_gradient_string.default.rainbow(`${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)} MB`)}][${import_gradient_string.default.rainbow(`${info.timestamp}`)}] ${info.level} [${import_gradient_string.default.pastel.multiline("XtonCore")}] [${info.label}] [Client] : ${info.message}`
        )
      )
    }),
    new import_winston.transports.File({
      ...
      format: import_winston.format.combine(
        import_winston.format.printf(
          (info) => `[${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)} MB]${info.timestamp} [${info.label}] [XtonCore] ${info.level} [Client] : ${info.message}`
        )
      )
    })
  ]
...
function registerCommands(_0) {
  return __async(this, arguments, function* ({
    ...
    const applicationCommands = yield getAppCommands(client, guild);
...
var ClientHandler = class {
  constructor({
    ...
    (0, import_figlet.default)("XtonCore", function(err, data) {
      if (err) {
        Clientlogger.error("Something went wrong...");
        console.dir(err);
        return;
      }
      console.log(import_gradient_string2.default.rainbow(data));
