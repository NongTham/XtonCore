...
var filename = import_path3.default.join(logDir, `Client.log`);
var errorFilename = import_path3.default.join(logDir, `Error.log`);
var debugFilename = import_path3.default.join(logDir, `Debug.log`);
var Clientlogger = (0, import_winston.createLogger)({
  level: process.env.NODE_ENV === "production" ? "info" : "debug",
  format: import_winston.format.combine(
    import_winston.format.errors({ stack: true }),
    import_winston.format.label({ label: import_path3.default.basename(process.mainModule?.filename ?? "unknown") }),
    ...
    new import_winston.transports.Console({
      format: import_winston.format.combine(
        ...
        import_winston.format.printf((info) => {
          ...
          const timestamp = import_gradient_string.default.rainbow(`${info.timestamp}`);
          const core = import_gradient_string.default.pastel.multiline("XtonCore");
...
${info.stack}`;
  ...
  exceptionHandlers: [
    new import_winston.transports.File({ filename: import_path3.default.join(logDir, "exceptions.log") })
  ],
  rejectionHandlers: [
    new import_winston.transports.File({ filename: import_path3.default.join(logDir, "rejections.log") })
  ]
});
...
var PerformanceManager = class {
  ...
  constructor() {
    ...
    this.statsFile = import_path4.default.join(process.cwd(), "stats", "command-stats.json");
    this.metrics = {
  ...
  async initializeStatsDirectory() {
    try {
      await import_promises2.default.mkdir(import_path4.default.dirname(this.statsFile), { recursive: true });
    } catch (error) {
    ...
    try {
      ...
      for (const filePath of componentFiles) {
        try {
          ...
          const fileURL = (0, import_node_url2.pathToFileURL)(absolutePath).href;
  ...
  watchDirectory(directory, callback, options = {}) {
    ...
    try {
      ...
      const watcher = import_fs2.default.watch(directory, { recursive }, async (eventType, filename2) => {
        if (!filename2)
          return;
        const filePath = import_path6.default.join(directory, filename2);
        const ext = import_path6.default.extname(filename2);
...
var ClientHandler = class {
  ...
  async _initialize() {
    try {
      ...
      if (figletData) {
        console.log(import_gradient_string2.default.rainbow(figletData));
      }
  ...
  async _eventsInit() {
    ...
    for (const eventPath of eventPaths) {
      const eventName = import_path7.default.basename(eventPath);
      ...
      this._client.on(eventName, async (...args) => {
        for (const eventFuncPath of eventFuncPaths) {
          try {
            const absolutePath = import_path7.default.resolve(eventFuncPath);
            const fileURL = (0, import_node_url3.pathToFileURL)(absolutePath).href;
  ...
  async _validationsInit() {
    ...
    for (const validationFilePath of validationFilePaths) {
      try {
        const absolutePath = import_path7.default.resolve(validationFilePath);
        const fileURL = (0, import_node_url3.pathToFileURL)(absolutePath).href;
